...
<body>
  <div class="side">
    ...
    <?php foreach ($data['orderviewDetails'] as $orderviewDetails) { ?>
      <?php if ($orderviewDetails->sellerID == $_GET['seller_id'] && $data['customerDetails']->customer_id == $orderviewDetails->customer_id) { ?>
    ...
    <div class="AR">
      <form action="<?php echo URLROOT; ?>/drivers/Accepted" method="post">
        <input type="hidden" name="seller_id" value="<?php echo $_GET['seller_id'] ?>">
        <input type="hidden" name="order_id" value="<?php echo $data['order_id'] ?>">

        <button class="accept" type="submit">Delivered </button><!-- <button class="deliver">Delivered</button> -->
      </form>
